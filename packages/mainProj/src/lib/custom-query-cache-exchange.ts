import { pipe, share, filter, merge, makeSubject, map } from "wonka"
 ...
 * 2. 离线时自动从缓存返回查询结果
 * 3. cache-only 模式下严格只返回缓存，不发送网络请求
 * 4. 不处理 mutation 队列和自动重试
 *
 * 使用场景：
 * - 需要查询缓存在后端宕机时仍然可用
...
export const customQueryCacheExchange =
  ...
  (input) => {
    ...
    if (storage && storage.readData && storage.writeData) {
      ...
      // 拦截 cache-only 请求，阻止其继续传递到网络层
      const forward: ExchangeIO = (ops$) => {
        return pipe(
          ops$,
          // 过滤掉 cache-only 的查询操作，不让它们传递到 fetchExchange
          filter((op: Operation) => {
            if (op.kind === "query" && op.context.requestPolicy === "cache-only") {
              // cache-only 查询不应该传递到网络层
              // cacheExchange 已经处理过了，如果缓存命中会直接返回
              // 如果到达这里说明缓存未完全命中，我们不应该发送网络请求
              console.log("[customQueryCacheExchange] 拦截 cache-only 查询，不发送网络请求:", op.key)
              return false
            }
            return true
          }),
