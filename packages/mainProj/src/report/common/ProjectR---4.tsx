...
import {useFrameEditorBar, useFormFramework} from "@/report/hook/useFormFramework";
...
import { Edit, Trash2, Plus, Save, X, AlertCircleIcon } from "lucide-react"
         ...
         <div className="h-md:@md:max-w-[80rem] m-auto">
             <Card className="py-1 gap-2">
                ...
                <CardContent className="p-0 space-y-1">
                 ...
                 {/*因为加border-t了导致CardFooter生效 [.border-t]:pt-6 */}
                 <CardFooter className="flex flex-col justify-end border-t px-2 !pt-1 gap-2">
                     <div>
                        <span className="text-sm">有些是不在附页中体现的但却在目录中有的其页号需设定。想清空所有项目（分项）和目录的配置（谨慎使用！）</span>
                        <Button size="sm" onClick={clearProjectCatalog}>重新初始化</Button>
                     </div>
                     {render()}
                 </CardFooter>
            </Card>
