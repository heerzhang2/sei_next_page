...
import { Badge, Card, CardContent, CardFooter, CardHeader, CardTitle, Label } from "@/components/ui"
    ...
    return (
        <>
            <PrintReserveLeast
                ...
                title={
                    <>
                        <h2 id={tag} className="text-2xl text-center mt-4">
                            {title}
                        </h2>
                        ...
                        <TableRow className="border border-gray-700">
                            <TableCell colSpan={2} className="border border-gray-700">
                                <RepLink ori rep={rep} tag={tag}>
                                    <div>
                                        {orc.简图说明 && <span className="text-sm whitespace-pre-wrap">{orc.简图说明 || "／"}</span>}

                                        {!(orc?._FILE_S简图?.length > 0) && !orc.简图说明 && (
                                ...
                                {orc?._FILE_S简图?.map(({ name, url }: any, i: number) => {
                                    return (
                                        <div key={i} className="break-inside-avoid-page pb-[1px] pt-[1px] overflow-hidden">
                                            ...
                                            <JumpTab
                                                key={i}
                                                href={`/rep/${rep?.id}/${rep?.modeltype}/${rep?.modelversion}/BoilerDiagram?#BoilerDiagram_pf${i}`}
                                            >
