            ...
            <CollapseFx printMode={printMode} subrid={subrid}>
                ...
                <FlexibleTable columnWidths={["17%","18%","12%","12%","16%","9%","%"]}>
                    <TableBody>
                        <TableRow>
                            <CCell>检测标准</CCell>
                            <TableCell  className="border border-gray-700" colSpan={6}>GB/T 11344-2021《无损检测 超声测厚》<br/>
                                    NB/T 47013.3-2015 《承压设备无损检测 第3部份：超声检测》</TableCell>
                        </TableRow>
                            ...
                            <TableCell colSpan={7} className="border border-gray-700">
                                <RepLink ori rep={rep} tag={'TkmsDiagram'} subrid={subrid} redId={redId}>
                                    ...
                                    {orc.点图说明 || '／'}
                                        ...
                                        {!(orc?._FILE_S部位?.length>0) && !orc?.点图说明 && <span className="block m-4 text-xl text-center">空的，进入上传吧</span>}
                                    </div>
                                </RepLink>
                                {orc?._FILE_S部位?.map(({name, url}: any, i: number) => {
                                    return <div key={i} className="break-inside-avoid-page pb-[1px] pt-[1px] overflow-hidden">
                                        {i>0 && <hr/>}
                                        <JumpTab key={i} href={`/rep/${rep?.id}/${rep?.modeltype}/${rep?.modelversion}/TkmsDiagram?original=1${apds}${apdr}#FxDiagram_pf${i}`}>
                        ...
                        <RepLink ori rep={rep} tag={'TkmsMeasurement'} subrid={subrid} redId={redId}>
                            <TableRow>
                                {(new Array(6).fill(null)).map((a:any,i:number) => {
                                    return (
                                        <React.Fragment key={i}><CCell className="text-sm">测点编号</CCell><CCell className="text-sm">测点厚度</CCell></React.Fragment>
                                    );
                                } )  }
                            </TableRow>
                        </RepLink>
...
export const config测厚表=[['测点编号','n',125],['测点厚度','v',75]] as Each_ZdSetting[];
export const TkmsMeasurement = ({children, show, label, rep,config=config测厚表,
                                    subrid, redId,modType }: ThkPartSummaryProps) => {
    const {storage,setStorage,subrType,modified,setModified} =useStorage();
    const subStore=storage?.[`_${modType}_${redId}`];
    const schema = React.useMemo(() => {
        const schemaFields = {} as any
    ...
    const onConfirm = useCallback((form: UseFormReturn<any, any, any>) => handleConfirm(), [])
    const { render,handleConfirm,form,arrayControls } = useFormFramework({schema, defaultValues, arrayFields, rep,
        subrid, redId, modType,
    })
    const [nestRenderer]=useTableEdit({form,arrayControls, config: config, table:'测厚表',onConfirm,
        externalData: subStore,defFixedLay:true, headview,tailview, pageSize:10
    });
    const content = React.useMemo(() => {
            return (
...
export const TkmsConclusion =
({
        rep,
        children,
        show = true,
        label, subrid, redId, modType,
    }:InternalItemProps
) => {
