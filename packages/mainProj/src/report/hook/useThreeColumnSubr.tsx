 ...
 * 报错Invalid prop `onClick` supplied to `React.Fragment`. React.Fragment can only have `key` and `children` props.
 * 报错Invalid prop `columnWidths` supplied to `React.Fragment`. React.Fragment can only have `key` and `children` props.
 * src\app\rep\[repId]\INDPL_DJ\1\layout.tsx (24:51) @ Layout
 * */
export function useThreeColumnSubr({orc, config, parentOrc, split, slash}
   : {orc: any,config:any[][][], parentOrc: any, split?:number[],slash?:boolean}
) {
    ...
    const parts= newCfgs?.map(( config : any[][][], i:number)=> {
        ...
        return  config?.map(([[desc,name,cb],add2p,thirdP] : any, k:number)=> {
...
/**三列组合的 可编辑 检验概况 ，可类比usePrefixDataReviewTbl，这里是支持3列的；【可以】支持拆分区块，支持嵌入列；
 * @property  orc ：记录数据。
 * @property  split : 支持按照一行做拆分config的，分成2半，或更多分区作独立输出分解的。默认是整个输出的。 split[]的内容是config数组的0开始的顺序序号必须排序好了。意思：拆分到了为止，序号后面的行要分离出去。
 * @return  node parts[] ：不拆分的只有一个[node] ,拆分个数不能动态。 可直接按行分解变身几个部分！每个独立再来拼装的。
 * @param slash  需斜杠替换空白的。的可能多个;(规定分成6列)
