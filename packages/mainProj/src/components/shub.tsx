...
{ orc?._FILE_测点?.url &&
    <img src={process.env.NEXT_PUBLIC_OSS_ENDP+orc?._FILE_测点?.url || "/placeholder.svg"} alt={orc?._FILE_测点?.url}
         css={{
             maxHeight: '14cm',   //在这个元素的上一级组件可以自己知晓的。打印高度自己决定的。这个用固定高度会导致图片自动的横竖比例不均衡压缩=会变形啊！24cm是纸张大约最多高度=报告最大图片高。
...
export const ImageComponent: React.FC<ImageProps> = ({
    ...
    return (
        <div className={cn("flex justify-around items-center", divClass)} style={{ ...style } as any}>
            <img
                src={src || "/placeholder.svg"}
                alt={alt}
                className={cn(
                    "object-contain max-h-[15cm] @md:5xl:max-h-[19cm]",
    ...
    return (
        <div style={{ position: "relative" }}>
            <button
                ...
                data-slot="switch"
                className={cn(
                    "peer data-[state=checked]:bg-primary data-[state=unchecked]:bg-input focus-visible:border-ring focus-visible:ring-ring/50 dark:data-[state=unchecked]:bg-input/80 inline-flex shrink-0 items-center rounded-full border border-transparent shadow-xs transition-all outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50 h-[25px] w-[42px] [&>span]:h-[21px] [&>span]:w-[21px] [&>span]:data-[state=checked]:translate-x-[17px]",
                    className,
                )}
                id={id}
                onClick={handleClick}
...
/*配套 useForm， 简化嵌套的形式；
 * */
export function FormHybridSelect({
                                     field,
                                     label,
                                     options,
                                     defSel,
