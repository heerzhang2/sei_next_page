    ...
    const createClientStable = useCallback(() => {
        ...
        const cache = offlineExchange({
            ...
            keys: {
                ...
                Report: (data: any) => data.id || null,
                User: (data: any) => data.id || null,
            },
            storage: {
                ...storage,
                readMetadata: async () => {
                    try {
                        ...
                        if (networkStatusContext && !networkStatusContext.isGraphQLBackendReachable) {
                            console.log("[offlineExchange] 后端不可达，跳过读取离线队列")
                            return []
                        }
                ...
                writeMetadata: async (json: SerializedRequest[]) => {
                    ...
                    try {
                        // 检查后端是否可达 - 如果后端不可达且传入空数组，则不覆盖现有队列
                        if (!json?.length && networkStatusContext && !networkStatusContext.isGraphQLBackendReachable) {
                            console.log("[offlineExchange] 后端不可达且传入空数组，保留现有队列")
                            return // 不执行任何操作，保持现有队列
                        }
                        ...
                        if (!json?.length) {
                            ...
                            if (typeof window !== "undefined") {
                                ...
                                // Clear graphcache data to prevent circular references
                                try {
                                    await storage.writeData!({})
                                    console.log("[offlineExchange] 已清空缓存数据以防止循环引用")
                                } catch (cacheError) {
                                    console.warn("清空缓存数据失败:", cacheError)
                                }
                        ...
                        const handleOnline = async () => {
                            ...
                            if (!isInitializing) {
                                ...
                                setTimeout(async () => {
                                    try {
                                        await storage.writeData!({})
                                        console.log("[offlineExchange] 网络恢复时已清空缓存数据")
                                    } catch (error) {
                                        console.warn("网络恢复时清空缓存失败:", error)
                                    }
                                    callback()
                                }, 100)
                            } else {
                                console.log("[offlineExchange] 初始化阶段忽略网络恢复事件")
                            }
                        }
            ...
            updates: {
                Mutation: {
                    modifyOriginalRecordData: (result, args, cache, info) => {
                        cache.invalidate({ __typename: "Report", id: args.id })
                    },
                },
            },
        })
...
const INITIALIZATION_DELAY = 5000 // 1 second delay to ensure proper offline detection
