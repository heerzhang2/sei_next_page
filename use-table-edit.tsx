...
小屏幕大屏幕和浏览器适应需要，多加个表头独立做复制的。实际表格数据行可能在屏幕上多行输出的。但是对于字段书很少的表格，也会出现大屏幕上有多个表格数据行在同一行上做并排布局情况。
...
export function useTableEdit({
        ...
        (form: UseFormReturn<any, any, any> | null, arrays?: Record<string, any>) => {
            ...
            return (
                <Card
                    ...
                    ref={editorRef}
                >
                    <div className="flex justify-between items-center sticky top-0 bg-background z-10 p-2 border-b md:pr-96">
                        {noDelAdd && seq === null ? null : <div>在{seq === null ? "新增一" : `编辑第 ${seq! + 1} `}条：</div>}
                        {showEditorPortal && <>
                            <Button type="button" className="ml-auto" variant="outline" size="sm" onClick={() => {
                                onConfirm(form!);
                                handleCloseEditor()
                            }}>
                                <Check className="h-4 w-4 mr-2" />确认
                            </Button>
    ...
    // 添加一个函数来确保表单数据同步
    const syncFormDataToEditor = React.useCallback(
        (rowIndex: number) => {
            if (rowIndex !== null && localTableData[rowIndex]) {
                const rowData = localTableData[rowIndex]

                // 遍历配置，设置表单值
                // config.forEach(([_, tag, __, ___, park]) => {
                //   if (park) {
                //     if (rowData[park]) {
                //       form.setValue(`${table}.${rowIndex}.${park}.${tag}`, rowData[park][tag] || "")
                //     }
                //   } else {
                //     form.setValue(`${table}.${rowIndex}.${tag}`, rowData[tag] || "")
                //   }
                // })
            }
        },
        [config, localTableData, table],
    )
