...
import {
    ...
    InputLine,
    Select,
    Popover, PopoverClose, PopoverContent, PopoverDescription, PopoverRefer, PopoverHeading,
 ...
 */
export const OneUserChoose= ({ name, oobj, setEditorVar, ...other }:OneUserChooseProps) =>
{
    const {user} = useContext(UserContext);
    const [open, setOpen] = React.useState(false);
    //模态对话框+点击触发加载数据的模式，类似于路由点击链接Relay PreloadData()模式。
    const [queryReference, loadQuery] = useQueryLoader(OneUserChooseQuery);
  ...
  return (
   ...
   <Popover open={open} onOpenChange={setOpen}>
       <PopoverContent>
           <PopoverHeading>用户选择相关</PopoverHeading>
           <PopoverDescription>
               <div >
                   <React.Suspense fallback="等下马上来...">
                       { queryReference && <OneUserChooseInner queryReference={queryReference} onSelect={handleSelect} oobj={oobj}/> }
                   </React.Suspense>
               </div>
           </PopoverDescription>
           <PopoverClose>
...
interface OneUserChooseInnerProps {
    queryReference: any;
 ...
 * __fragments: {OneUserChoose_User: {…}}  __id: "3hPum060QP6JWAlSxgD881VzZXI"
 ...
 * 需要通过对象setXxx()传递的情形，不能用fragment OneUserChoose_User on User来声明字段，要改成直接全字段声明，倒腾一手不认识! 和Relay没关系，因为react原因？
 * 举例，底下改成 dvs{id,name, offices{id,name, staff{...OneUserChoose_User} }, 后 组件状态管理无法直接透明传递了，setOffice(officeSel);不能office?.staff?.map((hit:any,来正常提取职员列表的User对象了！
 * 片段 ...OneUserChoose_User 不能在本组件直接使用，必须和其它Relay嵌套传递reference做法那样，在其它子孙组件利用Relay函数来接收和转换后才能正常显示。
 * 双层模态对话框，常规的PC端应用程序常用的模态编辑选择输入的方式。【对照的模式】采用路由页面附加URI参数解析和多个路由独立页面之间的状态数据传递的方式。
 * */
function OneUserChooseInner({ queryReference, onSelect, oobj }:OneUserChooseInnerProps)
{
    ...
    const { node: unit }=data as OneUserChooseQuery$data;
    ...
    const ixDep=user.dep && divisions!.findIndex((it:any) => it.id === user.dep!.id);
    const [dep, setDep] = React.useState<any>(ixDep && divisions?.[ixDep!]);      //才能够初始化底下的几个科室数据
        ...
        >
            {divisions?
                <React.Fragment>
                    ...
                    <InputLine label={`${office? '科室' : dep? '部门' : '单位'}下的人员:`}>
                        <Select inputSize="md" css={{minWidth:'140px',fontSize:'1rem',padding:'0 1rem'}} divStyle={css`max-width:240px;`}
                                value={people?.id || ''}
                                onChange={e => {
                                    const peopleSel= (office? office?.staff : dep? dep?.staff : unit?.staff)?.find((it:any) => it.id === e.currentTarget.value);
                                    setPeople(peopleSel);
                                } }
                        >
         ...
         <Layer elevation={"sm"}     css={{ padding: '0.25rem' }}>
             ...
             <Text variant="h5" css={{ textAlign: 'center' }}>
               选定：{people?.person?.name} 平台账户：{people?.username} 确定吗？
             </Text>
             <Button
                 intent="primary"
