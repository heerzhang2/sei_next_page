 ...
 * */
export function AgreementListItem(props: DeviceListItemProps) {
...
//若是需要切换编辑选中状态：可以动态的把<ListItem前边或者后边区域开启。正中间ListItem__content内容Div区域flex动态收缩。
  return (
      ...
      >
          <Text>{ eqp?.plno || '?' }</Text>
          <Itext>{ eqpTypeAllMap.get(det?.vart!) || eqpTypeAllMap.get(det?.sort!) }</Itext>
          <Itext>{ ( det?.cod || det?.oid) }</Itext>
