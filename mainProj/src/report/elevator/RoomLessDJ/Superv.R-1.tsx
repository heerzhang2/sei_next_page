...
//页面哈希路由方式
const JumpTags=[{name:'设备概况',ha:'Survey'},{name:'2机房（机器设备间）及相关设备',ha:'2.8'},
                 {name:'4轿厢与对重',ha:'4.8'},{name:'8 试验',ha:'8.1'},{name:'附录B：现场检验条件确认',ha:'Conclusion'}];
...
const OfficialReport: React.FunctionComponent<ReportViewProps> = ({
    ...
    const [rootMenu]=useRepMenuDirItems(JumpTags, [],`/reportView/ROL-DJ/ver/${verId}/${repId}`);
    const impressionismAs =React.useMemo(() => {
        return setupItemAreaRoute({verId, repId:repId!, theme});
    }, [verId, repId, theme]);
    const {renderIspContent} =useLikeElevatorOfficial({itRes:orc, orc,ItemArs:impressionismAs?.Item, model:'ROL-DJ',ver:verId, repId:repId!});
    ...
    <React.Fragment>
      ...
      >
       <div css={{
        ...
        { reportFirstPageHeadMa({theme,rep, mbbm:'FJB/TC-1001-2-2021'}) }
           <div css={{
               "@media print": {
                   display: 'flex',
          ...
          {注意事项({comply:'《电梯监督检验和定期检验规则——曳引与强制驱动电梯》（TSG T7001-2009）及1号、2号、3号修改单和《福建省电梯安全管理条例》制定，适用于无机房曳引驱动电梯定期检验',rep})}
          <DirectLink  href={`/report/ROL-DJ/ver/${verId}/${repId}/Instrument`}>
              <div css={{
                  display: 'flex',
                  flexDirection: 'column',
              ...
              <Text variant="h4">无机房曳引驱动电梯定期检验报告</Text>
              </div>
          </DirectLink>
        ...
        {报告设备详情({orc, repId:repId!, verId,rep})}
          ...
          <Table fixed={ ["3.5%","4%","8%","5%","13%","%","17%","8%"] }
              ...
              <TableHead>
                  <DirectLink  href={`/report/ROL-DJ/ver/${verId}/${repId}/ALL`}>
                      <TableRow>
        ...
        <div>
            <RouterLink href={`/report/ROL-DJ/ver/${verId}/${repId}/Instrument#Instrument`}>
                <Text variant="h4" css={{"@media print": {display: 'none'}}}>二、主要测量设备性能检查</Text>
            </RouterLink>
            <RouterLink href={`/report/ROL-DJ/ver/${verId}/${repId}/Conclusion#Conclusion`}>
                <Text id={'Conclusion'} variant="h4" css={{"@media print": {display: 'none'}}}>附录B：现场检验条件确认</Text>
            </RouterLink>
        </div>
      {末尾链接({template:'ROL-DJ',verId, repId: repId||''})}
    </React.Fragment>
  );
}
