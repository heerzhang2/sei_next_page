...
export const 报告设备详情= ( { orc,  repId, verId, rep } : { orc: any,repId: string,verId: string,rep:any}
  ...
  return <React.Fragment>
      <Table id={'Survey'} fixed={ ["6%","10%","30%","18%","%"] }  css={ {borderCollapse: 'collapse' } }>
        <TableBody>
          <DirectLink  href={`/report/ROL-DJ/ver/${verId}/${repId}/Survey#Survey`}>
            <TableRow>
            ...
            <TableRow>
              <CCell colSpan={2}>使用单位代码</CCell>
              <CCell colSpan={3}>{orc.使用单位信用码 || '／'}</CCell>
            </TableRow>
            <TableRow>
              <CCell colSpan={2}>安全管理人员</CCell>
              <CCell>{orc.安管人员 || '／'}</CCell>
              <CCell>单位内编号</CCell>
              <CCell>{orc.单位内部编号 || '／'}</CCell>
            </TableRow>
            <TableRow>
              <CCell colSpan={2}>制造日期</CCell>
        ...
        <DirectLink  href={`/report/ROL-DJ/ver/${verId}/${repId}/Conclusion#Conclusion`}>
          <TableRow>
            <CCell>检验结论</CCell><CCell colSpan={4}><Text variant="h1" css={{fontSize:'4rem'}}>{orc?.检验结论}</Text></CCell>
          </TableRow>
        </DirectLink>
       <DirectLink  href={`/report/ROL-DJ/ver/${verId}/${repId}/Witness#Witness`}>
          <TableRow>
            <CCell>备注</CCell>
