...
import {backItemsConfigSearch, RecordInputConfig, RecordIspArea, RecordSelfSplit} from "../common/config";
...
//检验项目的标准化展示组件
interface ActionMapItemLikeVehicleProps  extends React.HTMLAttributes<HTMLDivElement>{
    ...
    sureCB?: ({inp,setInp,tago, tagX,tagY,tagZ} :any)=>React.ReactNode;
}

// export const 工作见证的选择=['W','ZW','X','Z','XW','WX','ZXW'];
//【单一个编辑区域的】 全部项目。LikeVehicle
export const ActionMapItemLikeElevator=
    React.forwardRef((
        { children, show=true, alone=true,editAreasConf,index,refWidth,custST,sureCB}:ActionMapItemLikeVehicleProps, ref
    ) => {
                    ...
                    const rowHead =<div key={wf} css={{marginTop: '1rem'}}>
                        ...
                        { alikeSize<=0 && <>
                            ...
                            { selfSplit?  <>
                                    ...
                                    { sureCB? sureCB({inp,setInp,tago, tagX,tagY,tagZ})
                                         :
