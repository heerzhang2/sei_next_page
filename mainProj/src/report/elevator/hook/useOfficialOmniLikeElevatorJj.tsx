...
/**[有些局限性的 类似模板可采用这个] 正式报告的显示： 这种配置模式，显示逻辑变简单了，但是对配置的修改要求变麻烦了。
 * 项目编号栏目统一预备虚拟的四个列colSpan。 外部Table必须预留4列给项目栏目的。
 * [#关键的特殊#] 第2，4列是类别和nos独立显示， 第3列 第5列才是正常的项目栏目区域。 模型配置那一层可以不变，但是解析到报告项目展示那一层大变动。
 * @param itRes: 相比orc参数，差别是前者可能被测量数据替换掉部分检验结果栏目的显示数据。
 * 【特殊点】正常应该用 useOfficialOmniLikeCraneSi 函数： rpo:{span:0} 来让项目编码第二列不做显示，可实际这一行项目是要显示的。
 * 保留用法： #_span=-1 的是指示项目栏目前面的吃掉后面追随的更小级别的项目栏目Cell合并使用的，项目某级别栏目会扩充列占用空间的。对比的：rpo:{span:0}用来指示某一个级别栏目都不显示了。
 * 对于正式报告 span:0 的情形，需对对应的rec：{seco:'#',span:1}像这样的需要重载定义。
 * */
export const useOfficialOmniLikeElevatorJj= ({orc, ItemArs, model,ver, repId, itResCB} : Props
) => {
    ...
    const renderIspContent =React.useMemo(() => {
            ...
            area && area.items.forEach((et:ItemOmniConfig, n:number) => {
                if(et){
                    ...
                    if(et.name && (et.oRSpan>0 || splitHeadEt?.display) )        //归并自拆分头尾可能是纯文本的行，et.name=空的就跳过去。
                    {
                        ...
                        // else if(et.rpo.tspan) nosCc=3;  else
                        if(et.rpo.span) nosCc=2;
                        else nosCc=1;
                        let itemRowRender=[];
                        ...
                        if(et.offset){
                            ...
                            if(et.display)
                                ...
                                caption= mergLastEt.mergLabel??mergLastEt.recap??mergLastEt.tips;
                        }
                        else
                            caption= et.recap??et.tips??et.mergLabel;
                   ...
                   //【特别注意】底下((et.rpo.bspan!)>0) &&若改为{ (et.rpo.bspan) && <CCell rowSpan={0} colSpan={0}</CCell> } 若rpo.bspan===0将会导致异常，逻辑毛病输出<tr><td/>0<tr/>被当作0数字而非boolean?
                        ...
                        let secoColOcp=2===nosCc? 1: 1;       //第三个级别项目编码栏目 腾挪 td 给 标题用。
                        if(et.rpo.tspan===0){
                            secoColOcp=1;
                            captionOcp=3;
                        }
                        ...
                        //【这里特殊定义函数使用的地方还没有碰到】支持, 项目区的第一栏列吞并第二栏列的情形。
                        ...
                        const nos_Lable=(mergLastEt?.pre??et.pre??'') + (mergLastEt?.mergNos??mergLastEt?.nos??et.nos);
                        // console.log("行碎例",b,n,"nosCc=",nosCc,"ET=",et,'caption:',caption,'mergLastEt',mergLastEt,"倒退n-1行",area.items[n-1],splitHeadEt,"big_Spl",big_Spl);

                        itemRowRender[0] =<TableRow id={n===0 ? area.tag:undefined} key={n}>
                            <CCell key={1}>{seq}</CCell>
                            { et.oRSpan>=1 &&
                                <CCell split={true} key={10} rowSpan={et.rpo.span!>0? et.rpo.span:1}>{et.iclas}</CCell>
                            }

                            {((et.rpo.bspan!)>0) && <CCell key={2} split={true} rowSpan={et.rpo.bspan} colSpan={bigColOcp}
                                 >{big_Spl}</CCell>
                            }

                            { et.oRSpan>=1 &&
                                <CCell split={true} key={11} rowSpan={et.rpo.span!>0? et.rpo.span:1}>{nos_Lable}</CCell>
                            }
                            ...
                            <Cell key={6} colSpan={captionOcp}>{caption}</Cell>
                            <CCell key={7}>{result || '/'}</CCell>
                            { et.oRSpan>0 &&
                                    <CCell key={8} split={true} rowSpan={et.oRSpan??1}>{conseq || '/'}</CCell>
                            }
                        </TableRow>;
        ...
        return  htmlTxts;
    }, [orc,repId,model,ver,ItemArs,atPrint,itResCB]);
