...
//【单一个编辑区域的】 全部项目。
export const ActionMapItemLikeYyWt=
React.forwardRef((
    { children, show=true, alone=true,editAreasConf,index,refWidth,custST,sureCB,sureList,sureD}:Props, ref
) => {
        ...
        config.items?.forEach((tago, i) => {
            ...
            if(!tago.nconcl){       //归并的那一行位置。
                ...
                if(mergeName){
                    ...
                    if(sureD)
                        fields[`${mergeName}_S`]= par[`${mergeName}_S`];      //确认日期
                }
            }
        });
        if(custST)
    ...
    const render =React.useMemo(() => {
                ...
                const rowHead =<div key={wf} css={{marginTop: '1rem'}}>
                    ...
                    <LineColumn  column={5}>
                        ...
                        { sureCB? sureCB({inp,setInp,config,tago,addMemo,icname})
                            ...
                            sureD && addMemo
                            && <>
                                <InputLine label={`${tago.mergNos??tago.nos??''} 确认时间`}>
                                  <Input  value={(inp?.[`${icname}_S`]) || ''} type='date'
                                       onChange={e => setInp({ ...inp, [`${icname}_S`]: e.currentTarget.value || undefined})}/>
                                </InputLine>
                            </>
                        }
                    </LineColumn>
        ...
        return  htmlTxts;
    }, [config,inp,setInp, sureList,sureCB,sureD]);
