...
import {BlobInputList, Input, InputDatalist, InputLine, LineColumn, Text} from "customize-easy-ui-component";
import {RecordOmniArea} from "../common/omni";
import {工作见证的选择} from "../crane/hook/useRecordListSub";
...
//【单一个编辑区域的】 全部项目。
//【特殊点】“见证表”的编辑结果需作为项目列表编辑器中的工作见证的选择列表依据。
export const ActionMapItemLikeParkJj=
React.forwardRef((
        ...
        config.items?.forEach((tago, i) => {
            ...
            if(!tago.nconcl){       //归并的那一行位置。
                ...
                if(mergeName){
                    ...
                    fields[`${mergeName}_S`]= par[`${mergeName}_S`];      //确认方式
                }
            }
        });
        if(custST)
    ...
    const render =React.useMemo(() => {
                    ...
                    <LineColumn  column={5}>
                        ...
                        { addMemo && <>
                            <InputLine  label='工作见证'>
                                <InputDatalist value={(inp?.[`${icname}_Z`]) || ''} datalist={witnessNos}
                                               onListChange={v => {
                                                   inp[`${icname}_Z`] = v || undefined;
                                                   setInp({ ...inp });
                                               } } />
                            </InputLine>
                            <InputLine  label='确认方式'>
                                <InputDatalist value={(inp?.[`${icname}_S`]) || ''} datalist={工作见证的选择}
                                               onListChange={v => {
                                                   inp[`${icname}_S`] = v || undefined;
                                                   setInp({ ...inp });
                                               } } />
                            </InputLine>
                            <InputLine label={mergLabel+'-备注'}>
                                <Input value={(inp?.[`${icname}_M`]) || ''} onChange={e => {
                                    setInp({ ...inp, [`${icname}_M`]: e.currentTarget.value || undefined}); }} />
                            </InputLine>
                         </>
                        }
                    </LineColumn>
        ...
        return  htmlTxts;
    }, [config,inp,setInp, witnessNos,sureList]);
