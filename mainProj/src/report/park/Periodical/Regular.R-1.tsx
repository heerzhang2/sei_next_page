...
import {注意事项Mbcr} from "../../mobilecr/viewAttention";
...
const JumpTags=[{name:'设备概况',ha:'Survey'},{name:'2.资料和文件审查',ha:'2.1'},
...
const 检验结果替换 =((orc: { [x: string]: any; }) => {
    let out={...orc};
    out['绝缘电阻']=`绝缘电阻:${floatInterception(orc['绝缘阻o'],1,)??''}MΩ`;
    return out;
});
    ...
    <React.Fragment>
      {rootMenu}
       <div css={{"@media not print": {marginTop: '1rem', marginBottom: '1rem'}}}>
            ...
            {注意事项Mbcr({
                rep,
                comply: '是依据《起重机械安全技术规程》（TSG 51—2023），对起重机械进行定期检验的结论报告'
            })}
           ...
           {报告设备详情({theme, orc, rep})}
            ...
            <Table fixed={ ["3%","7%","8%","10.2%","8.1%","%","14.5%","6.2%","7%"]  } tight miniw={800}
           ...
           <Table fixed={ ["6%","40%","%"]  } printColWidth={ ["6%","40%","%"] }  css={ {borderCollapse: 'collapse' } }>
               <TableBody>
                   <RepLink rep={rep} tag={'Witness'}>
                       <TableRow>
                           <CCell>备注</CCell>
                           <Cell split={true} colSpan={2}><div css={{minHeight: '1rem', whiteSpace: 'pre-wrap'}}>
                               {orc.大备注 || '／'}
                           </div></Cell>
                       </TableRow>
                   </RepLink>
