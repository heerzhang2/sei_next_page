    ...
    <React.Fragment>
        <div css={{"@media not print": {marginTop: '1rem', marginBottom: '1rem'}}}>
            <div css={{
                ...
                <Text variant="h5" css={{textDecoration: 'underline'}}>FJJ/TK-1023-2023</Text>
            </div>
            <Text variant="h3" css={{
                textAlign: 'center',
                "@media (min-width:690px),print and (min-width:538px)": {
            ...
            {测量备注三半({orc, rep, config:config观测数据(orc),config2:config观测数据2,config3:config观测数据3,mem:'观备注',label:'八、观测数据及测量结果记录',children:tail观测})}

            {测量允许检测({orc, rep, config:config主技术,tag:'MainTechnical',mem:'主技备注',label:'附录A K7.5 主要技术参数测试',children:tail主技})}
            <StrainStressVw orc={orc} rep={rep} label={'附录B K7.6应力测试记录'}/>
            <AccelerationVw orc={orc} rep={rep}  stnum={3} label={'附录C K7.7加速度（A）检测记录'}/>

            {测量结果单位({orc, rep, config:config格栅流,tag:'Safetygap',mem:'格栅备注',runit:'(m/s)',label:'附录D K8.19 安全格栅间隙流速',children:' '})}
            {测量结果单位({orc, rep, config:config最驶速,tag:'Maxspeed',mem:'最驶备注',label:'附录E K8.25 碰碰船最大行驶速度',children:' '})}
            {常用现场条件({orc, rep, config: tItems现场,dcln:5,label:'附录F：现场检验条件确认'})}
        </div>
