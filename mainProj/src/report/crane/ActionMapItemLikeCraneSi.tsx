...
//【单一个编辑区域的】 全部项目。
export const ActionMapItemLikeCraneSi=
React.forwardRef((
        ...
        for(let wf=0; wf<config.items?.length; ){
            ...
            if(tago) {
                ...
                const rowHead =<div key={wf} css={{marginTop: '1rem'}}>
                    ...
                    { addMemo && <InputLine label={mergLabel+'-备注'}>
...
/*
{ tago.weight!==undefined && <div css={{ display: 'flex',flexDirection: 'column', justifyContent: 'space-around' }}>
    <Text>{weightTag}{`-权重(${tago.weight})得分 》`}</Text>
    <InputLine label={`得分`} labelTextStyle={css`font-size:1.6rem;`} >
        <Input type={"number"} value={(inp?.[`${tago.name}_F`]) || ''} onChange={e => {
