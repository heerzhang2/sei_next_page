...
import {
    Button, InputLine, Input, TextArea, Select, InputDatalist, Layer, Text, CheckSwitch, SuffixInput,
    IconCheckSquare, IconSquare, TableRow, CCell, useTheme, TableHead, TableBody, Table, Cell, ButtonRefComp, ColumnWidth,LineColumn,
...
export const ItemConclusionCr=
    React.forwardRef((
        ...
        const getInpFilter = React.useCallback((par: any) => {
            const {检验结论,新下检日,检验条件,环境,易燃,检验日期} =par||{};
            return {检验结论,新下检日,检验条件,环境,易燃,检验日期};
        }, []);
        const {inp, setInp} = useItemInputControl({ ref });
        const [floor, setFloor] = React.useState<string|null>(null);
        return (
            <InspectRecordLayout inp={inp} setInp={setInp}  getInpFilter={getInpFilter} show={show}
                                 alone={alone}  label={'四、结论； 附录11：现场检验条件'} column={0}>
                <Text  variant="h5">
                  附录11：现场检验条件确认
                </Text>
...
//【问题】测量结果表的结构化程度不好；有些项目特异性的排版的；假如用类似检验项目列表那样类似的配置模式，感觉不见得更好，可维护性较差，代码的部分删除或增加的栏目变动直观性不好，所见即所得。
...
export const Evaluation=
    React.forwardRef((
        ...
        return  <InspectRecordLayout inp={inp} setInp={setInp}  getInpFilter={getInpFilter}  show={show}  redId={redId} nestMd={nestMd}
                                       alone={alone}  label={'五、技术资料见证 六、备注'}>
            {render见证}
            注：首次检验、再次安装的流动作业的起重机械实施定期检验时填写该表格
            <hr/>
            <Text variant="h5">
                六、备注:
            </Text>
...
const item观测表绝=['主回阻','滑轮阻','车架阻','门架阻','交流感','直流感','空吸盘',];
const itemA观测表绝=['绝缘起r','空吸盘r','观测备注'];
export const ObservationInsula=
    React.forwardRef((
        ...
        return (
            <InspectRecordLayout inp={inp} setInp={setInp}  getInpFilter={getInpFilter} show={show}
                                 alone={alone}  label={'附录1：观测值及测量结果记录表C3.14 - C4.5.2.4'} column={0}>
...
const itemA安全距观=['安全距r','输线电压','安距备注',];
export const SafetyDistance=
    React.forwardRef(({ children, show ,alone=true}:InternalItemProps,  ref
        ...
        return (
            <InspectRecordLayout inp={inp} setInp={setInp}  getInpFilter={getInpFilter} show={show}
                                 alone={alone}  label={'附录2：C3.3安全距离'}>
...
export const Thickness=
    React.forwardRef(({ children, show ,alone=true,refWidth}:InternalItemProps,  ref
        ...
        return (
            <InspectRecordLayout inp={inp} setInp={setInp}  getInpFilter={getInpFilter} show={show}
                                 alone={alone}  label={'附录3：C3.7.3主要受力结构，附录4：C3.7.4梯子、走台和栏杆'}>
                ...
                2、未测量或无需测量的，仅填检验结果栏。
                <hr/>
                附录4：C3.7.4 梯子、走台和栏杆观测值及测量结果记录表
                {render梯子走台}
...
export const Monitoring=
    React.forwardRef(({ children, show ,alone=true,refWidth}:InternalItemProps,  ref
        ...
        return (
            <InspectRecordLayout inp={inp} setInp={setInp}  getInpFilter={getInpFilter} show={show}
                                 alone={alone}  label={'附录5：C4.2.2.5和C4.9.7安全监控管理系统验证'}>
                附录5：C4.2.2.5和C4.9.7安全监控管理系统参数验证表<br/>
...
export const Speed=
    React.forwardRef(({ children, show ,alone=true}:InternalItemProps,  ref
        ...
        const [getInpFilter]=useMeasureInpFilter([],itemA主起升速,(par)=>{
        ...
        return (
            <InspectRecordLayout inp={inp} setInp={setInp}  getInpFilter={getInpFilter} show={show}
                                 alone={alone}  label={'附录6：C4.3.2.1各机构运行速度记录表'}>
                附录6：C4.3.2.1各机构运行速度记录表<br/>
                {render主起升速}
...
export const Braking=
    React.forwardRef(({ children, show ,alone=true,refWidth}:InternalItemProps,  ref
        ...
        return (
            <InspectRecordLayout inp={inp} setInp={setInp}  getInpFilter={getInpFilter} show={show}
                                 alone={alone}  label={'附录7：C4.3.2.2起升机构制动距离，附录8：C4.3.2.3各机构同步性能'}>
...
export const Stiffness=
    React.forwardRef(({ children, show ,alone=true}:InternalItemProps,  ref
        ...
        return (
            <InspectRecordLayout inp={inp} setInp={setInp}  getInpFilter={getInpFilter} show={show}
                                 alone={alone}  label={'附录9：C4.3.2.5主梁静态刚度C4.4.2.4主梁跨中上拱度'}>
                附录9  C4.3.2.5 主梁静态刚度、C4.4.2.4主梁跨中上拱度和有效悬臂处上翘度测量记录<br/>
...
export const Magnetic=
    React.forwardRef(({ children, show ,alone=true,refWidth}:InternalItemProps,  ref
        ...
        return (
            <InspectRecordLayout inp={inp} setInp={setInp}  getInpFilter={getInpFilter} show={show}
                                 alone={alone}  label={'附录10：C4.9.8.1漏磁检查, 附录12：C3.4附设装置检验'}>
                {render漏磁检}
                ...
                附录12   C3.4附设装置检验项目<br/>
                附表12-1 桥架型起重机、臂架型起重机、电动葫芦等起重设备:
...
export const ItemRecheckResult=
    React.forwardRef((
        ...
        // const {renderIspContent} =useLikeCraneSupervision({itRes:orc,ItemArs:impressionismAs?.Item, model:'CR-DJ',ver:verId, repNo:`${repId}`});
        const getInpFilter = React.useCallback((par: any) => {
            const {unq} =par||{};
            return {unq};
        }, []);
