...
import {
  ...
  Evaluation,
  ObservationInsula,
  SafetyDistance, Thickness, Monitoring, Speed, Braking, Stiffness, StrainStress, Magnetic, ItemRecheckResult
} from "./orcBase";
import {createItem, } from "../../common/eHelper";
import {ActionMapItems, useRecordListSub} from "../hook/useRecordListSub";
...
const recordPrintList =[
  ...
  createItem('StrainStress', <StrainStress/>),
...
export const OriginalView=
  React.forwardRef((
      ...
      for(let p=0; p<oldItCount; p++){
        if(extendTags.indexOf(recordPrintList[p].itemArea)>=0){     //需要展开 扩充的标签
            ...
            itemConfigs.forEach((area, x) => {
              seq += 1;
              const rowHead =<ActionMapItems key={seq} alone={false} editAreasConf={itemConfigs} index={x}/>;
    ...
    // const urlPrintV=`/originalView/CR-JJ/ver/${verId}/`+ repId+'/printAll';
    // function newJob() {
