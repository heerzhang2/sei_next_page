 ...
 * */
export const useOfficialOmniLikeCraneSi= ({orc, ItemArs, model,ver, repId, itResCB} : Props
) => {
    ...
    const renderIspContent =React.useMemo(() => {
            ...
            area && area.items.forEach((et:ItemOmniConfig, n:number) => {
                if(et){
                   ...
                   //【特别注意】底下((et.rpo.bspan!)>0) &&若改为{ (et.rpo.bspan) && <CCell rowSpan={0} colSpan={0}</CCell> } 若rpo.bspan===0将会导致异常，逻辑毛病输出<tr><td/>0<tr/>被当作0数字而非boolean?
                        ...
                        // console.log("行碎例",b,n,"nosCc=",nosCc,"ET=",et,'caption:',caption,'mergLastEt',mergLastEt,"倒退n-1行",area.items[n-1],splitHeadEt);
                        ...
                        itemRowRender[0] =<TableRow id={n===0 ? area.tag:undefined} key={n}>
                            ...
                            { et.oRSpan>0 &&<>
                                    ...
                                    <CCell key={9} split={true} rowSpan={et.oRSpan??1}>{orc?.[icname+'_M']}</CCell>
                                </>
                            }
                        </TableRow>;
        ...
        return  htmlTxts;
    }, [orc,repId,model,ver,ItemArs,atPrint]);
