...
import {EvaluateItemConfig, itemResTransformEval, RecordEvalArea} from "../../safe/config";
import {ItemOmniConfig, itemResTransformRec, RecordOmniArea} from "../../common/omni";
import {resTranslCm} from "../../common/helper";

/** 【未完成】   格式化版原始记录 的生成。 类似评估配置模式的。
 * 对比正式报告： 原始记录的内容会多点的。 列数。
 *  试剂配制啊
 * */
export const useFormatOmniLikeCraneSi= ({itRes, ItemArs, model,ver, repNo, }
         :{itRes:any, ItemArs:RecordOmniArea[], model:string,ver:string, repNo:string,}
) => {
                ...
                if(et){
                    ...
                    itemRowRender[0] =<TableRow id={n===0 ? area.tag:undefined} key={n}>
                        ...
                        { et.name? <>
                            ...
                            :
                            <Cell key={6} colSpan={3}>{et.desc}</Cell>
                        }
                        {et.fRSpan && <>
