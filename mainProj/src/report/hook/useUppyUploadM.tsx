...
/**像管道单线图的这种上传：连续切换上传页面-多个上传状态机一起，切换毛病。单线图只能每一次管理对象对应一张图。
 * 【差异】出现URL点击切换，但是@uppy状态无法卸载再重新mount的情况下，才需要用这个的，否则用普通的useUppyUpload。且useUppyUploadM()不支持上传多张图
 * 修订：上传成功自动确认提交给storage存储，强制提示务必要保存当前storage给后端DB。
 ...
 * @return {} 节点DOM
 * */
export function useUppyUploadM({repId,table, field='_FILE_', inp, setInp, onFinish, row,liveDays=2,maxSize=5,idPref='Report' }
                     : {repId: string,table:string,field?:string, inp: any, setInp:React.Dispatch<React.SetStateAction<any>>,
        onFinish:(row:number,file:any,del:boolean)=>void, row:number, liveDays?:number,maxSize?:number,idPref?:string }
) {
    //若采用uppy.setState({ forRow: row });来做虽然可以区分保存，可是界面上的内容不能自动·切换清理旧的状态，感觉古怪。
    const theme= useTheme();
    //连续切换和上传的只能这样。多个 new Uppy() ;

    //需要建立一对一，每一行row重建uppy实例。 ”idPref+row“作为关键字。
    ...
    // const [tusID,setTusID] = React.useState();
    ...
    // const delayedSuccess = useCallback(
    //     (otusID: string) => {
    //         setOpenUppy(false);
    //         console.log("延迟知道handSURL row=",row,fileurls,"fileotusID",fileurls&&fileurls![otusID], otusID,"infiles",infiles);
    //         //if(!inp)    return; 竟然还没有准备inp？ 【问题大】handleUpSuccess会执行多次，多个组件混淆。
    //         // onFinish(row,newfile?.[0] || undefined, false);
    //     },
    //     [row,infiles,fileurls, onFinish]
    // );
    // function delayedSuccess(otusID: string) {
    //     setOpenUppy(false);
    //     console.log("延迟知道handSURL row=",row,fileurls,"fileotusID",fileurls&&fileurls![otusID], otusID,"infiles",infiles);
    //     // resourceArrvieed([]);
    // }
    ...
    const handleUpSuccess = React.useCallback((result: { successful: any[]; }) => {
        ...
        if(cntfile>0){
            if(!(newarr?.[cntfile-1]?.url)){
             //   setTusID(newarr?.[cntfile-1]?.uploadURL);

                // console.log("没找到url，只能去设置tusID=",newarr?.[cntfile-1]?.uploadURL,"OLD-", tusID, "超过1文件不成立了",newarr);
                // clearTimeout(timerRef.current);        //若没加定时器的直接打印会把当前弹出的菜单项列表一起显示出来的。
                // timerRef.current = setTimeout(() => {
                //     delayedSuccess(newarr?.[0]?.uploadURL);        //不能直接用newPrWindow!.print(); 不是同一个APP环境的;
                // }, 4500);
                return;
            }
            setOpenUppy(false);
    ...
    // const resourceArrvieed = (fileIDs: string[]) => {
    //     return new Promise(resolve => {
    //         let newUppsta=uppy.getState();
    //         console.log("resourceArrvieed newUppsta=",newUppsta,fileurls,"fileotusID",fileurls&&fileurls![tusID!], tusID,"infiles",infiles);
    //         resolve(void 0);
    //     });
    // };
