...
小屏幕大屏幕和浏览器适应需要，多加个表头独立做复���的。实际表格数据行可能在屏幕上多行输出的。但是对于字段书很少的表格，也会出现大屏幕上有多个表格数据行在同一行上做并排布局情况。
...
export function useTableEdit({
  ...
  const editor = React.useCallback(
    (form: UseFormReturn<any, any, any> | null, arrays?: Record<string, any>) => {
      ...
      const handleCancel = React.useCallback((form : UseFormReturn<any, any, any>) => {
        form.reset();
        handleCloseEditor();
      }, [handleCloseEditor]);
            ...
            <div className="flex gap-2 ml-auto">
              {showEditorPortal && (
                <>
                  ...
                  <Button variant="ghost" size="sm" onClick={() => handleCancel(form!)}>
