...
export function useTableEdit({
  ...
  const scrollToEditor = React.useCallback(() => {
    setTimeout(() => {
      if (editorRef.current) {
        try {
          ...
          while (element.parentElement) {
            ...
            if (overflowY === "auto" || overflowY === "scroll") {
              ...
              // 滚动父元素，使编辑器居中
              element.scrollTop = element.scrollTop + relativeTop - (element.clientHeight / 2 - rect.height / 2)
        ...
        // 4. 如果找不到编辑器引用，滚动到页面底部作为后备方案
        window.scrollTo({
          top: document.body.scrollHeight,
          behavior: "smooth",
        })
      }
    }, 300) // 减少延迟时间，但仍然给DOM足够的时间更新
  }, [])
  ...
  const editor = React.useCallback(
    ...
    [seq, config, table, editAs, fixColumn, scrollToEditor, localTableData, handleTableOperation, form],
  )
