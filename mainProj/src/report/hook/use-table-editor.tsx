...
import { Check, Undo, EyeIcon as EyeClosed } from "lucide-react"
...
export function useTableEditor({
  ...
  const handleCancel = React.useCallback(
      (e: React.MouseEvent, form: UseFormReturn<any, any, any> | null) => {
        if (form && seq !== null) {
          if (isEditingNewRow && form.control._formValues[table] && form.control._formValues[table].length > seq) {
            // 如果是新增或插入的行，直接移除
            const { remove } = form.control._names.array[table]
            if (remove) {
              remove(seq)
      ...
      [seq, config, table, isEditingNewRow, externalData],
  )

  // 将 handleTableOperation 移到顶层
  ...
  const clearTable = React.useCallback(
      ...
      [defaultV, table, handleTableOperation, onConfirm],
  )

  // 将 handleCloseEditor 移到顶层
  const handleCloseEditor = React.useCallback((e: React.MouseEvent) => {
    setShowEditorPortal(false)
    setSeq(null)
  ...
  // 将 handleAddNewRecord 移到顶层
  const handleAddNewRecord = React.useCallback(
      (e: React.MouseEvent, form: UseFormReturn<any, any, any> | null, arrays?: Record<string, any>) => {
        e.preventDefault()
        const { append } = arrays?.[table] || {}
        if (append) {
          // 如果当前有选中行，复制该行数据
      ...
      [seq, localTableData, config, table, handleTableOperation],
  )
  ...
  // 修改 handleHeaderPosition 函数，使用浮动表头而不是移动原始表头
              ...
              >
                <div className="flex justify-between items-center sticky top-0 bg-background z-10 p-2 border-b @md:pr-[5rem] @4xl:pr-[33rem]">
                  ...
                  <div className="flex gap-2 ml-auto">
                    <>
                      <Button variant="ghost" size="sm" onClick={(e) => handleCloseEditor(e)}>
                        <Check className="h-4 w-4 mr-2" />
                        关闭
                      </Button>
                      <Button variant="default" size="sm" onClick={(e) => handleConfirmEdit(e, form)}>
                        <EyeClosed className="h-4 w-4 mr-2" />
                        同步
                      </Button>
                      <Button variant="ghost" size="sm" onClick={(e) => handleCancel(e, form)}>
                ...
                <div className={cn("w-full", isCenterMode ? "flex-1 overflow-y-auto p-4" : "")}>
                  ...
                  {!noDelAdd && !showEditorPortal && (
                      <Button className="mt-1" onClick={(e) => handleAddNewRecord(e, form, arrays)}>
                        新增一条
                      </Button>
                  )}
                </div>
  ...
  // 在 renderCollapsibleTable 函数中添加浮动表头组件
  ...
  const renderTable = React.useCallback(
      (form: UseFormReturn<any, any, any> | null, arrays: Record<string, any>) => {
        const { fields } = arrays?.[table] || {}
        const linecnt = Math.ceil(fields?.length / raft)
        return fixedColWState ? renderCollapsibleTable(form, arrays, linecnt) : renderFlexibleTable(form, arrays, linecnt)
      },
      [fixedColWState, raft, renderCollapsibleTable, renderFlexibleTable, table],
  )

  // 监听 fixedColWState 变化，关闭浮动表头
  React.useEffect(() => {
    closeFloatingHeader()
  }, [fixedColWState, closeFloatingHeader])
