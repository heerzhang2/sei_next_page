...
import {useFormatOmniLikeMobCrIn} from "../hook/useFormatOmniLikeMobCrIn";
...
export const FormatOriginal: React.FunctionComponent<ReportViewProps> = ({
    ...
    }, [rep, verId, repId, theme]);
    const {renderIspContent} =useFormatOmniLikeMobCrIn({itRes:orc,ItemArs:impressionismAs?.Item, rep});
    ...
    <React.Fragment>
      ...
      >
        ...
        <div css={{
          ...
          <Table fixed={ ["2%","5%","5%","5%","3%","%","3%","3%","4%","6%"] } css={ {borderCollapse: 'collapse' } } tight  miniw={800}>
              <TableHead>
          ...
          {测量结果记录({orc, rep, label:'附录1 观测值及测量结果记录表'})}
          <SafeDistance orc={orc} rep={rep} label={'附录2：C3.3 安全距离观测值及测量结果记录表'}>
