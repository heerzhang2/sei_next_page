...
const config设备 = [
  ...
  [['改造单位名称','_$改造单位'], ['改造日期', {n:'_$改造日期',t:'d'}] ],
  [['维护保养单位名称', '_$维保单位']],
    //设备技术参数
  [['名义速度', '_$运行速度', 'm/s'], ['名义宽度', '_$名义宽度', 'mm'], ['倾斜角', '_$倾斜角度', '°'] ],
    //若用 , {n:'_$输送能',u:'P/h'},  #_$不被 useThreeColumnView识别
  [['输送能力', '_$输送能', 'P/h'],  ['提升高度', '_$提升高度', 'm'], ['使用区段长度', '_$使用区长度', 'm'] ],
];
//拆分成2个编辑器的
...
export const 报告设备详情= ( {theme, orc, rep } : { orc: any,rep:any, theme:any}
  ...
  const manyConl=orc?.检验结论?.length>20;
  return <React.Fragment>
    ...
    <Table fixed={ ["4.8%","9%","25%","9%","21%","12%","%"] }  css={ {borderCollapse: 'collapse' } }  tight  miniw={800}>
      <TableBody>
        <RepLink rep={rep} tag={'Survey'}>
          {firstPart}
        </RepLink>
        ...
        <DirectLink href={`/report/${rep?.modeltype}/ver/${rep?.modelversion}/${rep?.id}/Conclusion#Conclusion`}>
          <TableRow>
            <CCell>检测结论</CCell><CCell colSpan={6}><Text variant="h1" css={{fontSize: manyConl? '1.5rem':'2.4rem'}}>{orc?.检验结论}</Text></CCell>
          </TableRow>
        </DirectLink>
    ...
    {检验编制核准EscaTest({orc,rep})}
  </React.Fragment>;
};

export const 检验编制核准EscaTest= ({ orc,rep } : { orc: any, rep:any }
) => {
