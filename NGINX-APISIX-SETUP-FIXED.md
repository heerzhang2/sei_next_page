...
- APISIX 运行在 Docker 网络中，默认网络名称为 `docker-apisix`
...
# 复制修改脚本
cp /mnt/c/home/sei_next_page/disable-apisix-ssl-port.sh .

# 转换行尾符
sed -i 's/
$//' disable-apisix-ssl-port.sh

# 执行修改脚本
chmod +x disable-apisix-ssl-port.sh
./disable-apisix-ssl-port.sh
```
...
如果您看到类似 `$'\r': command not found` 的错误，这是由于 Windows 行尾符（CRLF）与 Linux 行尾符（LF）不兼容导致的。
...
```bash
# 查看 Docker 网络
docker network inspect docker-apisix

# 检查 APISIX 容器是否在正确的网络中
docker inspect apisix | grep NetworkMode
```
